...
public class BlockFT extends Block implements ITileEntityProvider {

    public static final PropertyBool active = PropertyBool.create("active");

    public BlockFT(){
        super(Material.PISTON);
        setUnlocalizedName(Reference.ExtremeEnergyBlocks.FT.getUnlocalizedName());
        setRegistryName(Reference.ExtremeEnergyBlocks.FT.getRegistryName());
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (!worldIn.isRemote){
            ...
            ModUtils.setState(worldIn, pos, state.withProperty(BlockFT.active, active));
            TileEntityFT te = (TileEntityFT) worldIn.getTileEntity(pos);
    ...
    public void breakBlock(World world, BlockPos pos, IBlockState blockstate) {
        TileEntityFT te = (TileEntityFT) world.getTileEntity(pos);
        try {
            te.brokeLinksAll();
            if (!te.isEmpty())
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            playerIn.openGui(ExtremeEnergy.instance, ModGuiHandler.FT_GUI, worldIn, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }
