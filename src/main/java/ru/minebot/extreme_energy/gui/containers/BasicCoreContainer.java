...
public class BasicCoreContainer extends BasicContainer {
    ...
    public BasicCoreContainer(IInventory player, InventoryBasicCore te){
        super(player, te, 1);

        addSlotToContainer(new ModuleSlot(te, te, 0, 80, 22, 0));

        // Player Inventory, Slot 9-35, Slot IDs 9-35
        for (int y = 0; y < 3; ++y) {
            for (int x = 0; x < 9; ++x) {
                this.addSlotToContainer(new Slot(player, x + y * 9 + 9, 8 + x * 18, 71 + y * 18));
            }
        }

        // Player Inventory, Slot 0-8, Slot IDs 36-44
        for (int x = 0; x < 9; ++x) {
            this.addSlotToContainer(new Slot(player, x, 8 + x * 18, 129));
        }
        ...
        inventorySlots.set(index, new DontTakeSlot(player, index - 28, 8 + (index - 28) * 18, 129));
    }
}
