...
public class ImplantsRadialMenu extends Gui {
    private int selected = -1;
    public int itemSelect = -1;
    public int lastX = 0;
    public int lastY = 0;

    public void draw(Minecraft mc, ImplantData data){
        ...
        for (int i = 0; i < stacks.size(); i++){
            ...
            mc.getTextureManager().bindTexture(new ResourceLocation("meem:textures/items/modules/" + stacks.get(i).getItem().getRegistryName().getResourcePath().substring(4)+".png"));
            drawModalRectWithCustomSizedTexture(-8, -8,0,0, 16, 16, 16,16);
            if (actives[i] == 0)
                GL11.glColor4f(1, 0, 0, 0.6f);
            else
                GL11.glColor4f(0, 1, 0, 0.6f);
            mc.getTextureManager().bindTexture(new ResourceLocation("meem:textures/gui/implantshud/moduleframe.png"));
            drawModalRectWithCustomSizedTexture(-8, -8,0,0, 16, 16, 16,16);
